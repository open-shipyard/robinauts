...
function chosenFrom(items: Agent[], kept: string | null): string {
  const first = items[0];
...
export function AgentPicker({ agents }: { agents: Agents }) {
  ...
  if (agents.status === "loading") {
    return <p className="text-sm text-muted">Loading the agents…</p>;
  }
  ...
  const chosen = chosenFrom(agents.items, kept);
  return (
    <label className="flex items-center gap-2 text-sm text-muted">
      ...
      <select
        value={chosen}
        onChange={(event) => {
          setKept(event.target.value);
          remember(AGENT_KEY, event.target.value);
        }}
        className="rounded-ui border border-edge bg-paper px-2 py-1 text-ink"
      >
