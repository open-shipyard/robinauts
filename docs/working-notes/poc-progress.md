...
- The licence gate is `scripts/licence_gate.py`, standard library only. It
  ...
  the tree.
- `frontend/` — the skeleton and its gates, and nothing else yet: Vite,
  React 19, TypeScript in strict mode, Vitest on jsdom, ESLint, Prettier.
  The page is a heading; the shell, the history and the chat are the steps
  after this one. `src/api/client.ts` is a typed `fetch` over the types
  ...
  prove every one of them still fires. `src/tokens.css` is a stub for the design
  tokens step 18 brings. The JavaScript gates: `vite.config.ts` carries the
  ...
  **Known limit:** that record is written from rollup's module graph, so a
  ...
  `url(pkg/x.png)`, a package stylesheet's own imports) is in the bundle and
  not in the list — it is still held to the allowed list by the tree gate
  below, and a reviewer checks CSS `@import`/`url()` targets by hand
  (`DEPENDENCIES.md`, "What the bundle's record does not see"). A scanner for
